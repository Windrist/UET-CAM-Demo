...
class App(QMainWindow):
    def __init__(self):
        ...
        # Save Data
        self.date, self.time = readFile.get_date_time()
        readFile.save_current_time(self.date, self.time)
    ...
    def initUI(self):
        ...
        self.uet_pixmap = QPixmap('logo/uet.png').scaled(111,111,Qt.KeepAspectRatio)
        ...
        self.exit_icon = QIcon('logo/close.jpg')
    ...
    def update_statistic(self, data):
        ...
        if data == "1":
            ...
            self.textBox.insertPlainText("Linh Kiện Tray {}".format(tray_idx+1) + " Hàng {}".format(col+1) + " Cột {}".format(row+1) + " Hoạt Động Tốt!\n")
        elif data == "0":
            self.number_error3 += 1
            self.tray[tray_idx].item(row,col).setBackground(QColor(232, 80, 91))
            self.textBox.insertPlainText("Linh Kiện Tray {}".format(tray_idx+1) + " Hàng {}".format(col+1) + " Cột {}".format(row+1) + " Bị Hỏng!\n")
        elif data == "-1":
            self.number_error1 += 1
            self.tray[tray_idx].item(row,col).setBackground(QColor(255, 255, 51))
            self.textBox.insertPlainText("Linh Kiện Tray {}".format(tray_idx+1) + " Hàng {}".format(col+1) + " Cột {}".format(row+1) + " Gặp Lỗi Vị Trí Trên Jig. Đề Nghị Kiểm Tra!\n")
        elif data == "404":
            self.number_error2 += 1
            self.tray[tray_idx].item(row,col).setBackground(QColor(255, 128, 0))
            self.textBox.insertPlainText("Linh Kiện Tray {}".format(tray_idx+1) + " Hàng {}".format(col+1) + " Cột {}".format(row+1) + " Gặp Lỗi Kết Nối Với Bộ Test. Đề Nghị Kiểm Tra!\n")
    ...
    def main_process(self):
        if self.command == "Idle":
            ...
            if self.get_cap_check == True:
                ...
                check = checkAlign.check(checkAlign.calc_mean(image))
                self.update_check_image(image) # Đưa video lên giao diện
                if check:
